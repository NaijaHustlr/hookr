...
const Story: React.FC<StoryProps> = ({ model, hasNewStory = false }) => {
  return (
...
const FeedPost: React.FC<FeedPostProps> = ({ post, model }) => {
  ...
  return (
    <Card className="mb-5 bg-hookr-dark border-hookr-muted/30">
      {/* Header */}
        ...
        <div className="flex items-center gap-3" onClick={handleProfileClick}>
          ...
          <div>
            <div className="flex items-center">
              ...
              {model.verified && (
                <Badge variant="outline" className="ml-1.5 h-4 py-0 px-1 border-hookr-accent/50">
      ...
      <div className="p-3">
        ...
        {/* Comments */}
        {post.comments > 0 && (
          <p className="text-xs text-hookr-light/50">
  ...
  return (
    <div className="flex flex-col min-h-screen pb-16 bg-hookr-dark">
      ...
      <header className="sticky top-0 z-30 bg-hookr-dark border-b border-hookr-muted/20 px-4 py-3 flex justify-between items-center">
        <h1 className="text-2xl font-bold text-hookr-light font-serif italic">hookr</h1>
        ...
        <Carousel className="w-full">
          <CarouselContent className="-ml-2">
            {mockModels.map((model) => (
              <CarouselItem key={model.id} className="pl-2 basis-auto">
      ...
      <Tabs defaultValue="feed" className="w-full" onValueChange={setActiveTab}>
        ...
        <TabsContent value="feed" className="mt-2 px-4 pb-4">
          {mockPosts.map(post => (
            <FeedPost key={post.id} post={post} model={getModelForPost(post)} />
          ))}
        </TabsContent>
        ...
        <TabsContent value="subscribed" className="mt-2">
          {activeTab === "subscribed" && (
            <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
              ...
              <Button 
                className="bg-hookr-accent text-white hover:bg-hookr-accent/90"
